...
| D18 | **Labels carry identity; `agent_status` carries liveness — never conflate them.** Merge finds a worktree's pane as the runtime pane whose label equals that worktree name; dispatch counts occupied slots from `herdr pane list --workspace <id>` using each pane's `agent_status`/`agent_session`, not its label. A runtime pane with **no label**, or with a label but a dead session, is an anomaly: dispatch neither claims it as free nor trusts it as occupied — it reports it to the chat pane. No state file, no registry, no channel between the control agents. | A `label` is pane metadata held by herdr and outlives the agent that set it — a stale `PBI-040` label on a dead pane would otherwise hold a slot forever. Everything both agents need is in herdr's live view, which cannot drift the way a separate registry would; `foreground_cwd` corroborates the label. |
