...
| D7 | Slice 1 delivers the **backlog-dispatcher only**. Slice 2 is the **report-only** merge-watcher and is inside this feature. Auto-merge is neither slice — it is a later feature, gated by D3. | Sequencing only: the dispatcher is what makes the loop useful, and the watcher without auto-merge is a report the user can also get on demand. |
...
- **PBI-039 gates auto-merge**, which is a later feature — not slice 2. The report-only watcher of slice 2 is unaffected (D3, D7).
