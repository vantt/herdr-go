...
|----|----------|-----------------------------------------------|
| D1 | A PBI is **dispatchable** iff all four hold: (a) a `docs/history/<slug>/CONTEXT.md` exists whose `**Backlog:**` line names that PBI id, (b) its `docs/backlog.md` row status is exactly `in-flight`, (c) no worktree grant exists for `<slug>`, (d) the feature has zero cells. The PBI→slug map is the **reverse index built by reading the `**Backlog:** PBI-NNN` line of each `docs/history/*/CONTEXT.md`** — backlog rows carry no slug, so the mapping cannot go the other way. No new column, no prose parsing of the Ghi chú cell. | Existing CONTEXT.md means the item passed Gate 1, the only step in the chain that produces new product decisions; everything after it is machine-verifiable. The status is `in-flight`, not `proposed`, because exploring itself flips the row when it opens the feature — "has a CONTEXT.md" and "is still `proposed`" are disjoint sets in this repo. Conditions (c) and (d) are what keep the dispatcher from re-picking work already under way. Zero schema migration; the `proposed/in-flight/done` enum stays three-valued (scribing D6 forbids a fourth status). |
...
| D4 | Both control panes run continuously. Where herdr exposes an event, they **wait on the event** rather than poll — `herdr wait agent-status <pane> --status …` and `herdr wait output <pane> --match …`. The **60-second poll** remains only for file-based signals herdr cannot push (new/changed backlog rows and CONTEXT.md files). Every agent, control and working alike, runs Claude on model **Sonnet**. | Polling for something herdr will tell you is wasted work and adds up to a minute of latency per hop; polling for a file change is the only option left. |
...
| D17 | **Panes name themselves.** The first act of any agent in this system is `herdr pane current --current` to learn its own `pane_id`, then `herdr pane rename <pane_id> <label>`: the control agents label themselves `dispatch` and `merge`; a working agent labels its pane with its **worktree name**. No outside process assigns labels. | Self-naming makes the labels authoritative rather than aspirational — a pane is named by the thing actually running in it, so a label can never describe a pane whose agent died or was replaced. It also removes the ordering problem of naming a pane before its occupant exists. |
| D18 | **Labels are the shared state.** Dispatch counts free slots and merge finds a worktree's pane by reading `herdr pane list --workspace <id>`; the runtime pane whose label equals a worktree name *is* that worktree's pane. No state file, no registry, no channel between the control agents. | The loop already coordinates through free slots (D15); labels extend that to identity. Every fact both agents need is derivable from herdr's live view, which cannot go stale the way a separate registry would. `pane list` also carries `agent_status` and `foreground_cwd` as corroboration. |
...
- `herdr wait agent-status <pane_id> --status idle|working|blocked|done|unknown [--timeout MS]`, `herdr wait output <pane_id> --match <text> [--regex] [--timeout MS]` — event waits instead of polling (D4).
...
- `herdr pane rename <pane_id> <label>|--clear` — self-naming (D17); `herdr pane list --workspace <id>` returns the labels that carry the shared state (D18).
...
- [ ] Precisely which signal ends a working agent's life — `herdr wait agent-status --status idle` (herdr's own view of the pane) or D2's four bee conditions — and how the two are reconciled when they disagree. The mechanisms both exist (D4); which is authoritative is a planning question.
