...
| D23 | **Extends D3.** On a red-stop, the merge role captures `bee worktree merge --cleanup`'s full output to `.bee/tmp/herdr-orchestrating.merge-out.<slug>`, deletes it on a green result, and on `MERGE_CONFLICT`/`MERGE_VERIFY_RED` moves it to `.bee/tmp/herdr-orchestrating.red.<slug>.log` beside the existing red-stop marker, naming that path in the chat-pane report. | The chat pane's report is a one-liner by design (D3); the actual conflict/verify detail previously existed only in the ephemeral merge pane's own scrollback, gone by the time a human reads the report. This also clarifies (no behavior change) that the **chat pane is itself a live interactive Claude session rooted at MAIN checkout** — investigating a red-stop does not need a separate "merge agent," and clearing the marker is the only re-entry point: the existing 60-second poll picks the worktree up again next cycle, there is no separate retry action. |
| D24 | `control-loop.sh` (both `.claude/` and `.agents/` copies, kept byte-identical) prints a timestamped, color-coded banner around each iteration (role, start/done/failed/timed-out), auto-disabled when stdout is not a TTY or `NO_COLOR` is set. | Purely cosmetic — no change to exit codes, control flow, or the `--command` test hook. The loop's own scaffolding lines were blending with `claude -p`'s own output in the pane, making an iteration's boundary hard to see. |

### Amendment (2026-07-22) — D23/D24 made as direct edits, not through the gated pipeline

D23 and D24 were implemented as direct edits on main during a conversation, not through `bee-exploring` → planning → validating → swarming. The user flagged this process gap mid-session and, given both changes are small/cosmetic/additive with no behavior-contract change, explicitly chose to let the direct edits stand rather than revert and redo them through the backlog pipeline. Recorded here so this divergence from the normal gated flow reads as a deliberate call, not an oversight. All three decisions (D23, D24, and this amendment) are also logged in the durable decision log (`bee decisions log`, tags `merge`/`red-stop`/`observability`, `control-loop`/`display`, and `process`/`direct-edit` respectively).
