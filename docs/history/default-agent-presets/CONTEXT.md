...
| D6 | ~~Default preset **Codex**: `{"label": "Codex", "argv": ["codex", "--full-auto"]}`~~ — **superseded by D10** (planning discovery found `--full-auto` deprecated). | User-selected initially; overturned same-turn once planning's L1 verification surfaced new evidence, per D10. |
| D10 | Default preset **Codex**: `{"label": "Codex", "argv": ["codex", "--sandbox", "danger-full-access", "--ask-for-approval", "never"]}` — full permission/sandbox bypass, symmetric with D5 (Claude) and D7 (Agy). | Supersedes D6. Planning's L1 discovery (WebSearch against developers.openai.com) found `--full-auto` deprecated since Codex CLI v0.128 — still runs today as a compatibility flag but docs steer toward explicit `--sandbox`/`--ask-for-approval` flags, and OpenAI could remove the old flag entirely in a future release, silently breaking this preset on every fresh install. Presented to the user with 3 concrete options (keep deprecated flag / modern equivalent of the old meaning / full bypass matching D5+D7); user chose full bypass for symmetry. |
...
- [x] ~~Confirm exact Codex CLI flag spelling before writing the cell~~ — resolved: see D10 (supersedes D6). `--full-auto` deprecated; locked to `--sandbox danger-full-access --ask-for-approval never`.
