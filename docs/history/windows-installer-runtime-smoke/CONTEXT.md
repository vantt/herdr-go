...
| D5a (2026-07-23) | Amendment, not a reversal: the real wait timed out on the first 3 real release runs against `v0.1.3`/`v0.1.4`/`v0.1.5` (widened 90s→180s made no difference; `Get-ScheduledTaskInfo` showed `LastRunTime` never advancing and `LastTaskResult` stuck at `1` -- Task Scheduler never attempted a restart at all). This matches D4's already-accepted gap from the other direction: a `LogonType=Interactive` task's restart-on-failure needs a real interactive logon session, which the headless `windows-2022` runner does not have. The step now soft-fails (`::warning::` + diagnostics, continues to the uninstall proof) instead of failing the release job. | Two release cycles blocked on a check that (per external Task Scheduler documentation) cannot succeed in this specific CI environment regardless of wait length -- confirming real end-user crash-restart behavior needs a real, interactively-logged-on Windows machine, tracked as follow-up rather than a release blocker. |
