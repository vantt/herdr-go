# NTM Inventory — 2026-07-15

Source: `/home/vantt/projects/research/multiplexer/upstreams/ntm` (Named Tmux Manager), HEAD `5840f61`.
Mechanical extraction only — no porting judgments, no cross-source comparison.
...
- **Where:** `/home/vantt/projects/research/multiplexer/upstreams/ntm/SKILL.md`, `references/COMMANDS.md`, `references/ROBOT-MODE.md`, `references/DASHBOARD.md`, `references/CONFIG.md`
...
- **Where:** `/home/vantt/projects/research/multiplexer/upstreams/ntm/command_palette.md`
...
- **Where:** `internal/rotation` (`Provider` interface: `Name/LoginCommand/ExitCommand/AuthSuccessPatterns`), `internal/swarm` (`account_rotator.go`)
...
- **Where:** `/home/vantt/projects/research/multiplexer/upstreams/ntm/AGENTS.md`
...
- **Where:** `/home/vantt/projects/research/multiplexer/upstreams/ntm/CHANGELOG.md`
...
- **Where:** `/home/vantt/projects/research/multiplexer/upstreams/ntm/UPGRADE_LOG.md`
...
- **Where:** `docs/planning/PLAN_TO_ADD_WEB_UI_AND_REST_AND_WEBSOCKET_API_LAYERS_TO_NTM__{CLAUDE_WEB,GEMINI,GPT,GPT_PRO,OPUS}.md`, `docs/planning/PROPOSED_HYPERSYNC_SPEC__{CODEX,OPUS}.md`
...
- **Where:** `internal/plugins` (sdk.go/agent.go/command.go: `SDKVersion`, `Capability` — inferred, no doc comment), `internal/cli/root.go` (`plugins.LoadCommandPlugins(cmdDir)`)
...
- **Where:** `/home/vantt/projects/research/multiplexer/upstreams/ntm/.goreleaser.yaml`
...
- **Where:** `/home/vantt/projects/research/multiplexer/upstreams/ntm/install.sh`
...
- **Where:** `docs/planning/PLAN_TO_ADD_WEB_UI_AND_REST_AND_WEBSOCKET_API_LAYERS_TO_NTM__{CLAUDE_WEB,GEMINI,GPT,GPT_PRO,OPUS}.md`
...
**Read in full:** `README.md`, `SKILL.md`, `command_palette.md`, `AGENTS.md` (914 lines), `UPGRADE_LOG.md`, `CHANGELOG.md` (1017 lines), all 22 top-level `docs/*.md` spec files named in the task brief, all 20 `docs/robot-*.md` files, all 12 files under `docs/planning/`, `docs/parity_matrix.json` (structural), `docs/openapi.json`/`docs/openapi-kernel.json` (structural), `Makefile`, `.goreleaser.yaml`, `install.sh`, `cmd/ntm/main.go`, `internal/cli/root.go` (command-registration section).

**Skimmed/structural only:** `docs/parity_matrix.json` and both `openapi*.json` files were inspected for shape/scale (path counts, tag distribution) rather than enumerated endpoint-by-endpoint, per the task's doc-first guidance.

**Package-level pass:** All 106 `internal/` package directories were enumerated via `ls` and their package doc comments (or, absent that, file-name inference) captured; 18 packages had no doc comment (`audit, auth, backpressure, bd, caut, clipboard, cm, encryption, ideation, kernel, palette, plugins, rotation, sqliteutil, summary, supportbundle, swarm`, plus `safety` which has no `.go` files of its own — only its `redaction/` subpackage). Go implementation internals were not read beyond doc comments / file-name inference, per the task's instruction not to deep-read Go code. Not every one of the 106 packages received its own dedicated feature entry above — the ones with limited standalone documentation (`clipboard`, `codeblock`, `notify`, `output`, `prompt`, `tokens`, `models`, `quota`, `ratelimit`, `updater`, `util`, `webhook`, `caut`, `sqliteutil`) are generic infrastructure and were folded into the closest domain narratively rather than given individual entries, to avoid diluting the report with one-line restatements of their doc comments; their one-line purposes are preserved verbatim in the raw sub-agent research this report was compiled from (available on request) if a finer-grained pass is later needed.

**Notable cross-cutting fact (not tied to one domain):** Several "RATIFIED" spec docs directly contradict each other despite sharing the same bead ID — three SQLite runtime-schema docs under `bd-j9jo3.2.1` (see [[context-memory]]), two freshness-contract docs under `bd-j9jo3.1.4` (different `SourceHealthEntry.Status` enums: `healthy/degraded/unavailable` vs. `fresh/stale/unavailable/unknown`), and two error-taxonomy docs under `bd-j9jo3.1.7` (see [[tooling]]). This is recorded as an observed fact about the doc corpus's internal consistency, not evaluated further.

**Proposed new domains?** None strictly required — everything found mapped to an existing taxonomy domain, though several features are legitimately cross-domain and were cross-referenced with `[[domain]]` links rather than duplicated in full (e.g. `cm`/CASS Memory spans context-memory and self-improvement; the file-reservation simulator spans orchestration and testing-evals; HyperSync could equally be filed under a hypothetical `distributed-storage` domain but was kept under orchestration since its stated purpose is distributing *agents* across machines, not storage in isolation).

**Unresolved/notable items surfaced during extraction (not evaluated, just flagged):**
1. `docs/schemas/` is referenced normatively in `docs/robot-schema-versioning.md` as the checked-in JSON-Schema layout but does not exist in the repo — a documented-but-unimplemented convention.
2. `docs/planning/PLAN_TO_IMPROVE_NTM_PROJECT.md`'s own table of contents promises 4 sections (CAAM/CM/SLB/Agent-Mail-existing-integration) that never appear in the document body.
3. Five Web-UI plans and two HyperSync specs are all still unimplemented, parallel, and in several places mutually contradictory — the repo currently keeps all variants rather than a single decided design.
