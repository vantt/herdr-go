...
> **Hội tụ độc lập lần 3 (2026-07-28) — "ack ≠ delivered" từ collie**: collie (Bun/TS, agent-framework khác hoàn toàn, cùng dùng herdr socket) phát hiện lại đúng cơ chế "gõ chữ ≠ submit" từ 1 sự cố production thật (#34: reply gửi lúc dialog focus trả lời nhầm dialog, mất message, ack vẫn báo success) — và tổng quát hoá thành race-guard dùng chung cho MỌI loại dialog gửi, không chỉ 1 trường hợp ([→](sources/collie.md#harness-race-guard-verify-before-submit)). Còn xác nhận thêm 1 điểm khớp với `HERDR_API.md`: field `revision` là stub luôn-0 trên herdr 0.7.x, không dùng được làm change-detector — cả collie's guard lẫn `HERDR_API.md`'s own ghi chú đều tự tránh dựa vào nó, phải re-derive nội dung để so sánh tươi. 3 nguồn độc lập (airemote, collie, herdr's own docs) cùng hội tụ trên 2 quirk giống nhau của cùng 1 API — Evidence tier 3, xem porting-log `send-confirm-submit-pattern`.
