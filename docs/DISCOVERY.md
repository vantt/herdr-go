...
**Residual — resolved 2026-07-20 (PBI-003, live probe against herdr 0.7.4, isolated session, evidence: `.bee/spikes/pane-agent-status-changed-live-probe/findings.md`):** driving a real throwaway agent pane through `working`/`blocked`/`idle` via real `pane.report_agent` calls (both sequentially and in a zero-delay rapid-fire burst) and measuring against a dedicated per-pane `events.subscribe` connection found:
- **Latency is not a blocker.** Every driven transition delivered in 11-18ms — roughly 30-40x faster than the current 500ms poll.
- **The 2×/1ms duplicate was not reproduced** under either driving method (sequential, ~300ms apart, or 5 rapid-fire calls with no delay). Instead, rapid-fire updates showed a different behavior: only 4 of 5 back-to-back transitions arrived as distinct events, spaced ~100ms apart — server-side coalescing on a roughly 100ms cadence rather than per-request emission (a potential silent-coalesce-of-a-sub-tick-state risk, not a duplicate risk; the existing cursor de-dup would still architecturally collapse any duplicate that does occur, for any event kind).
- **`done` never appears as its own `agent_status` value on `pane.agent_status_changed`.** Reporting `idle` on an unfocused ("unseen") pane delivered `agent_status: "idle"`, not `"done"`; a subsequent `pane.focus` (marking it "seen") with no new state report produced zero further events. This confirms DISCOVERY.md's original suspicion live, specifically via the subscribe channel (not just CLI/snapshot): the internal `pane_agent_status(state, seen)` idle-vs-done distinction does not surface on this event at all. A subscribe-based consumer must derive "done" itself from an `idle` transition, the same way the CLI-facing code already does.
- **Recommendation: qualified GO** for a future poll→subscribe migration of agent-status notify, carrying forward the two caveats above (derive "done" from "idle"; be aware of possible sub-~100ms coalescing). This spike does not itself implement that migration (out of scope, `src/watcher.rs` untouched).
