...
| web | axum: token+cookie auth (fail-closed, silent-404), switcher API, create routes (open a shell / start an agent, each destination carrying its resolved-folder provenance), and the **observe/reply** surface (poll a pane's screen, post a reply). No live PTY — herdr's API can't size a terminal to a phone (DISCOVERY 2026-07-18). | `src/web/` |
