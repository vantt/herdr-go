...
| agent orchestration (dev tooling, not product) | `.claude/skills/herdr-orchestrating/{SKILL.md,scripts/*,references/*}` (+ `.agents/` copy) | control-loop driver, lane classifier, cockpit/runtime builder, recorded spawn + dry-run proofs | `docs/specs/agent-orchestration.md` (partial; dispatch side built, merge side not yet) |
| tests (e2e) | `tests/observe_reply_e2e.rs` | real-server observe/reply proofs | — |
| installation | `install.sh`, `install.ps1`, `config.example.json`, `dev-deploy.sh`, `.github/workflows/{release,ci}.yml`, `scripts/windows-runtime-smoke.ps1`, `scripts/windows-install-smoke.ps1`, `scripts/macos-install-smoke.sh`, `scripts/generate-checksums.sh`, `scripts/update-smoke.sh`, `packaging/herdr-go.service`, `build.rs` | native roots and token lifecycle (`src/config/mod.rs`), `Assets` (`src/web/mod.rs`) | `docs/specs/installation.md` (partial); release-checksum publishing and the self-update smoke test are specced under `docs/specs/self-update.md` |
