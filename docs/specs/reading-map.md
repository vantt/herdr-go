...
| frontend | `web/src/{main,api}.ts`, `web/src/views/*.ts`, `web/src/styles.css` | `AgentRow`, `ShellRow`, `HealthInfo`, `TerminalProps`, `NewPaneRef` (post-create AND shell-entry navigation reference), `buildHomeGroups` (combines agent + shell rows), view renderers (poll-based, no WebSocket client) | switcher: `docs/specs/switcher.md` (partial; now covers shell entries on home); terminal detail: `docs/specs/terminal-detail.md` (partial); create sheet: `docs/specs/create-sheet.md` (partial; only missing screenshots); login not yet specced |
